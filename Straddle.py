...
#cost to buy
def straddle_date_cost(symbol,strikeprice,currentDate,expiryDate, index):
    costPrice=0
...
def straddle_month(startDate, expiryDate, index):
    cost =0
    symbol = 'NIFTY'
    ...
    costPrice=straddle_date_cost(symbol,strikeprice,startDate,expiryDate, index)
    ...
    while expiryDate >= evaluationDate:
        sellPrice = straddle_date_cost(symbol,strikeprice,evaluationDate,expiryDate, index )
...
def doStraddle(startDate, yearLimit,expiries):
    output = []
    year = startDate.year
    try:
        while year!=yearLimit:
            ...
            output.append(straddle_month(startDate,nextmonthexpiry, index =True))
