...
#cost to buy
def buyStrangle_date_cost(symbol,strikeprice,spreadBelow, spreadAbove,currentDate,expiryDate, index):
    costPrice=0
...
def buyStrangle_month(startDate, expiryDate, index):
    cost =0
    symbol = 'NIFTY'
    ...
    margin=0 #margin per sell option
    ...
    costPrice=buyStrangle_date_cost(symbol,strikeprice,belowSpread,aboveSpread,startDate,expiryDate, index)
    ...
    while expiryDate >= evaluationDate:
        sellPrice = buyStrangle_date_cost(symbol,strikeprice,belowSpread,aboveSpread,evaluationDate,expiryDate, index )
...
def dobuyStrangle(startDate, yearLimit,expiries):
    output = []
    year = startDate.year
    try:
        while year!=yearLimit:
            ...
            output.append(buyStrangle_month(startDate,nextmonthexpiry, index =True))
